...
class Bootstrap:
    ...
    def startup_engine(self):
        with self.app.app_context():
            processor = Processor(name=self.app.config['APP_NAME'], app=self.app, config=self.app.config)
            if not Engine.get_thread_by_name(processor.name):
                th = Engine(target=Engine, name=self.app.config['APP_ID'], processor=processor, interval=int(self.app.config['APP_SLEEP_TIME']))
                th.start()
