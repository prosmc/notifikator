...
class Logger(object):
    ...
    def init_app(self, app):
        log_type = app.config["APP_LOG_TYPE"]
        logging_level = app.config["APP_LOG_LEVEL"]
        if log_type != "stream":
            try:
                log_directory = app.config["APP_LOG_DIR"]
                app_log_file_name = app.config["APP_MAIN_LOG_NAME"]
                access_log_file_name = app.config["APP_WWW_LOG_NAME"]
        ...
        if log_type == "stream":
            ...
            log_max_bytes = int(app.config["APP_LOG_MAX_BYTES"])
            log_copies = app.config["APP_LOG_COPIES"]
